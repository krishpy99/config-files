...
return {
  s({"tdynamicmodularinteger", snippetType = "autosnippet"}, {
    t("
struct dynamic_mint {
	static int M;
	...
	dynamic_mint pow(long long n) const {
		dynamic_mint ans = 1, x(*this); for (; n > 0; n /= 2) {if (n & 1) ans *= x; x *= x;} return ans;
	}
	...
	dynamic_mint operator-() const {dynamic_mint m; m.val = (val == 0 ? 0 : M - val); return m;}
	...
	friend std::ostream &operator<<(std::ostream &os, const dynamic_mint &m) {return os << m.val;}

	friend std::istream &operator>>(std::istream &is, dynamic_mint &m) {long long x; return is >> x, m = x, is;}
};
